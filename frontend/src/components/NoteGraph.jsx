...
const GraphNoteNode = memo(({ data }) => {
  ...
  return (
    ...
    >
      ...
      <span>{data.backlinkCount} in · {data.outgoingCount} out</span>
    </button>
  );
});

const GraphToolbar = ({ onReset, scope, setScope, depth, setDepth, truncated }) => {
  ...
  return (
    <div className="graph-toolbar" aria-label="Graph controls">
      ...
      <div className="graph-toolbar__group">
        {truncated && <span className="graph-toolbar__note">Showing a limited graph</span>}
        <button type="button" className="text-button" onClick={() => { fitView({ padding: 0.2, duration: 350 }); onReset?.(); }}>
          Reset view
        </button>
      </div>
    </div>
...
const NoteGraph = ({ graph, currentNoteId, navigate, scope, setScope, depth, setDepth }) => {
  const nodes = useMemo(() => {
  ...
  if (!graph.nodes.length) {
    return <div className="graph-empty"><strong>No connected notes yet.</strong><span>Create or link notes to see your workspace relationships here.</span></div>;
  }
    ...
    <div className="graph-shell">
      ...
      <div className="graph-canvas" aria-label="Knowledge graph">
        ...
        >
          ...
          <MiniMap nodeColor={(node) => node.data?.isCurrent ? "#7ed39a" : "#496b55"} pannable zoomable />
        </ReactFlow>
      </div>
      <div className="graph-accessible-list" aria-label="Graph note relationships">
