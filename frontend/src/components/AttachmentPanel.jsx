...
const AttachmentPanel = ({ noteId }) => {
  ...
  const loadAttachments = useCallback(async () => {
    ...
    try {
      const response = await api.get(
        `/attachments/note/${encodeURIComponent(noteId)}`,
      );
  ...
  const upload = async (event) => {
    ...
    if (!file) return;
    if (!navigator.onLine) {
      toast.error("Attachments require a connection");
      return;
    }
    setStatus("uploading");
    ...
    try {
      ...
      setStatus("error");
      toast.error(
        error.response?.data?.message || "Could not upload attachment",
      );
    }
  };
  ...
  const remove = async (attachment) => {
    ...
    try {
      ...
      setAttachments((current) =>
        current.filter((item) => item._id !== attachment._id),
      );
  ...
  return (
    <section className="attachment-panel" aria-label="Note attachments">
        ...
        <div>
          ...
          <small>
            {offline
              ...
              : `${attachments.length} of 10 · 4 MB maximum each`}
          </small>
        </div>
        <button
          className="focus-toggle"
          onClick={() => inputRef.current?.click()}
          disabled={offline || status === "uploading"}
        >
          <Upload size={15} />{" "}
          {status === "uploading" ? "Uploading..." : "Add file"}
        </button>
        <input
          ref={inputRef}
          type="file"
      ...
      {status === "error" && (
        <p className="attachment-panel__message">
          Attachments are temporarily unavailable.
        </p>
      )}
      {status === "offline" && (
        <p className="attachment-panel__message">
          Large files are not stored in the offline note queue. Reconnect to
      ...
      {attachments.length > 0 && (
        <div className="attachment-list">
          {attachments.map((attachment) => (
            <article className="attachment-item" key={attachment._id}>
              ...
              <div className="attachment-item__details">
                ...
                <small>
                  {attachment.mimeType} · {(attachment.size / 1024).toFixed(0)}{" "}
                  KB
                </small>
              </div>
              <a
