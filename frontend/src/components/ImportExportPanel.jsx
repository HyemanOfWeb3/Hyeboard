...
const ImportExportPanel = ({ userId, onComplete }) => {
  ...
  const exportNotes = async (format) => {
    try {
      ...
      if (format === "json") {
        downloadText(
          "hyeboard-backup.json",
          JSON.stringify(createBackup(notes, trash), null, 2),
          "application/json",
        );
      } else {
        downloadText(
          "hyeboard-notes.md",
          createMarkdown(notes),
          "text/markdown",
        );
      }
      toast.success(
        format === "json" ? "Backup exported" : "Markdown exported",
      );
    } catch {
  ...
  const importFile = async (event) => {
    ...
    try {
      ...
      await onComplete?.();
      toast.success(
        `${result.imported.length} note${result.imported.length === 1 ? "" : "s"} imported${result.skipped.length ? `, ${result.skipped.length} skipped` : ""}`,
      );
    } catch (error) {
  ...
  return (
    <div className="portability-panel">
      ...
      {open && (
        <section
          className="portability-popover"
          aria-label="Import and export notes"
        >
          <div className="portability-popover__header">
            <div>
              <strong>Data portability</strong>
              <small>Exports use notes available on this device.</small>
            </div>
            <button
              className="icon-button"
              onClick={() => setOpen(false)}
              aria-label="Close portability menu"
            >
              <X size={16} />
            </button>
          </div>
          <div className="portability-actions">
            <button className="text-button" onClick={() => exportNotes("json")}>
            ...
            >
              <Download size={14} /> Markdown notes
            </button>
            <small className="portability-note">
              Attachments are independent stored assets and are not embedded in
              note exports.
            </small>
          </div>
          <div className="portability-import">
            <label htmlFor="import-duplicates">
              When a matching note exists
            </label>
            <select
            ...
            <button
              ...
              disabled={busy}
            >
              <Upload size={15} /> {busy ? "Importing..." : "Choose file"}
            </button>
            <input
              ref={inputRef}
              type="file"
          ...
          {progress && (
            <div className="portability-progress" role="status">
              <FileInput size={14} />{" "}
              {progress.total
