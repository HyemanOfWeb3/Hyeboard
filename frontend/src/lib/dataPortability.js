...
function cleanTags(value) {
  ...
  return Array.from(new Set(tags.map((tag) => cleanText(String(tag).trim(), "Tag", 120)).filter(Boolean))).slice(0, 50);
}
...
function normalizeRecord(record, index, isTrash = false) {
  ...
  const title = cleanText(record.title || "Untitled note", "Note title", 500).trim();
  const content = cleanText(record.content || "", "Note content");
  if (!title && !content) throw new Error(`Note ${index + 1} is empty`);
  const sourceId = cleanText(String(record.sourceId || record.clientNoteId || record.id || record._id || `import-${index + 1}`), "Note ID", 200);
  return {
    sourceId,
    ...
    updatedAt: cleanDate(record.updatedAt, cleanDate(record.createdAt, new Date().toISOString())),
    isPinned: Boolean(record.isPinned),
    isFavorite: Boolean(record.isFavorite),
    deletedAt: isTrash || record.deletedAt ? cleanDate(record.deletedAt, new Date().toISOString()) : null,
    internalLinks: parseInternalLinks(content).map(({ label, reference }) => ({ label, reference })),
  };
}
...
export function createBackup(notes = [], trash = []) {
  return {
    ...
    notes: notes.filter(Boolean).map((note, index) => normalizeRecord(note, index)),
    trash: trash.filter(Boolean).map((note, index) => normalizeRecord(note, index, true)),
  };
}
...
export function createMarkdown(notes = []) {
  return notes.filter(Boolean).map(serializeMarkdownNote).join(`\n\n${MARKDOWN_NOTE_MARKER}\n\n`);
}
...
function parseMarkdownBlock(block, index) {
  ...
  if (!match) throw new Error(`Markdown note ${index + 1} is missing valid frontmatter`);
  const frontmatter = {};
  match[1].split("\n").forEach((line) => {
    const separator = line.indexOf(":");
    if (separator < 1) throw new Error(`Markdown note ${index + 1} has invalid frontmatter`);
    frontmatter[line.slice(0, separator).trim()] = parseFrontmatterValue(line.slice(separator + 1), "");
  });
  return normalizeRecord({ ...frontmatter, content: match[2] }, index, Boolean(frontmatter.deletedAt));
}

export function parseImportText(text, fileName = "import") {
  if (typeof text !== "string") throw new Error("Import data must be text");
  if (new TextEncoder().encode(text).byteLength > MAX_IMPORT_BYTES) throw new Error("Import file is too large");
  ...
  if (isJson) {
    ...
    if (!parsed || parsed.format !== "hyeboard-backup" || parsed.schemaVersion !== BACKUP_SCHEMA_VERSION) {
      throw new Error("Unsupported HyeBoard backup format");
    }
    const notes = Array.isArray(parsed.notes) ? parsed.notes : [];
    const trash = Array.isArray(parsed.trash) ? parsed.trash : [];
    if (notes.length + trash.length > MAX_IMPORTED_NOTES) throw new Error("Import contains too many notes");
    return [...notes.map((note, index) => normalizeRecord(note, index)), ...trash.map((note, index) => normalizeRecord(note, index, true))];
  }
  const blocks = text.split(`\n\n${MARKDOWN_NOTE_MARKER}\n\n`);
  if (blocks.length > MAX_IMPORTED_NOTES) throw new Error("Import contains too many notes");
  return blocks.filter((block) => block.trim()).map(parseMarkdownBlock);
}

export function remapInternalLinks(content, idMap) {
  return content.replace(/\[([^\]]+)\]\(\/note\/([^)]+)\)/g, (match, label, reference) => {
