...
const CreatePage = () => {
  ...
  const goBack = () => {
    if (hasDraft && !window.confirm("Leave without saving this note?")) return;
    navigate("/");
  };
  const handleSubmit = async (event) => {
    ...
    try {
      await createNoteLocally(userId, {
        title: title.trim(),
        content,
        tags: normalizedTags,
      });
    ...
    <div className={`editor-page ${focusMode ? "editor-page--focus" : ""}`}>
      {!focusMode && (
        <header className="editor-header">
          ...
          <span className="editor-header__spacer" />
        </header>
      )}
      <main className="editor-main">
        <div className="editor-document-header">
          <div>
            <span className="eyebrow">Writing space</span>
            <h1>Capture the thought.</h1>
          </div>
          <button
        ...
        <form onSubmit={handleSubmit} className="editor-document">
          ...
          <footer className="editor-footer">
            <span>Draft stays here until you create it</span>
