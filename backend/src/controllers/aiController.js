...
function errorResponse(error) {
  ...
  const [status, message] = map[error.code] || [502, "AI could not complete the request"];
...
export async function askKnowledge(req, res) {
  try {
    ...
    const notes = await Note.find(query).select("_id title content tags updatedAt").sort({ updatedAt: -1 }).limit(80).lean();
    ...
    const sourceByKey = new Map(result.sourceKeys.map((key, index) => [key, sources[index]]));
    res.json({
      answer: result.answer,
      uncertainty: result.uncertainty,
      cached: result.cached,
      sources: result.sourceKeys.map((key) => sourceByKey.get(key)).filter(Boolean),
    });
  } catch (error) {
    const { status, message } = errorResponse(error);
