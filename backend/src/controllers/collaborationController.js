...
export const requireManageNote = requireNoteAccess({
  param: "noteId",
  manage: true,
});
...
export async function createInvitation(req, res) {
  try {
    ...
    if (!invitee)
      return res
        .status(404)
        .json({ message: "Invitee must have an existing HyeBoard account" });
    if (String(invitee._id) === String(req.noteAccess.note.user))
      return res
        .status(400)
        .json({ message: "The note owner already has access" });
    const existing = await NoteMember.findOne({
      note: req.noteAccess.note._id,
      user: invitee._id,
      revokedAt: null,
    });
    if (existing)
      return res.status(409).json({ message: "This user already has access" });
    const { token, tokenHash } = createInviteToken();
    const invitation = await NoteInvitation.create({
      note: req.noteAccess.note._id,
...
export async function listMembers(req, res) {
  const members = await NoteMember.find({
    note: req.noteAccess.note._id,
    revokedAt: null,
  })
    .populate("user", "email")
    .lean();
  res.json([
    { user: { _id: req.noteAccess.note.user }, role: "owner" },
    ...members,
  ]);
}
...
export async function acceptInvitation(req, res) {
  try {
    const invitation = await NoteInvitation.findOne({
    ...
    if (invitation.inviteeEmail !== req.user.email)
      return res
        .status(403)
        .json({ message: "This invitation belongs to another account" });
    await NoteMember.findOneAndUpdate(
      { note: invitation.note, user: req.user._id },
      {
        $set: {
    ...
    await writeCollaborationEvent({
      ...
      note: invitation.note,
      type: "member.changed",
      payload: { action: "accepted", role: invitation.role },
    });
...
export async function revokeMember(req, res) {
  const member = await NoteMember.findOneAndUpdate(
    { note: req.noteAccess.note._id, user: req.params.userId, revokedAt: null },
  ...
  await writeCollaborationEvent({
    ...
    note: req.noteAccess.note._id,
    type: "member.changed",
    payload: { action: "revoked", userId: String(member.user) },
  });
