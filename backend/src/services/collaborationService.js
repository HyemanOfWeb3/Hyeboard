...
export async function getNoteAccess(
  ...
  const note = await Note.findOne({
    ...noteReferenceFilter(noteReference),
    ...(includeDeleted ? {} : { deletedAt: null }),
  }).lean();
  if (!note) return null;
  if (String(note.user) === String(userId)) return { note, role: ROLES.OWNER };
  const member = await NoteMember.findOne({
    note: note._id,
    user: userId,
    revokedAt: null,
    acceptedAt: { $ne: null },
...
export async function accessibleNotesFilter(userId) {
  const memberships = await NoteMember.find({
    user: userId,
    revokedAt: null,
    acceptedAt: { $ne: null },
  })
...
export function requireNoteAccess({
  ...
  return async (req, res, next) => {
    try {
      ...
      if (!access || (!includeDeleted && access.note.deletedAt))
        return res.status(404).json({ message: "Note not found" });
      if (
        (edit && !canEdit(access.role)) ||
        (manage && !canManage(access.role))
      )
        return res
          .status(403)
          .json({ message: "You do not have permission for this note" });
...
export async function writeCollaborationEvent({
  actor,
  note,
  type,
  revision = null,
