...
const enabled =
  process.env.SEMANTIC_INTEGRATION === "true" &&
  Boolean(process.env.GEMINI_API_KEY || process.env.AI_API_KEY);
const options = {
  skip: !enabled
    ? "Set SEMANTIC_INTEGRATION=true with a disposable restricted Gemini key"
    : false,
};
...
test(
  "disposable Gemini embedding smoke test returns vectors",
  options,
  async () => {
    ...
    const response = await ai.models.embedContent({
      model,
      contents: "A note about offline synchronization",
    });
