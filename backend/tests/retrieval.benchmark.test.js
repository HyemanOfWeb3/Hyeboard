...
const notes = [
  {
    ...
    tags: ["security", "auth"],
    content: "Sessions use signed cookies and authenticated requests.",
    links: ["deployment"],
  },
  {
    id: "deployment",
    title: "Production Deployment",
    tags: ["vercel", "operations"],
    content: "Vercel environment variables and serverless deployment checks.",
    links: ["auth"],
  },
  {
    id: "offline",
    ...
    tags: ["offline", "sync"],
    content: "IndexedDB stores pending note mutations until reconnect.",
    links: ["conflicts"],
  },
  {
    id: "conflicts",
    ...
    tags: ["sync", "recovery"],
    content: "Revision checks preserve local and server versions.",
    links: ["offline", "history"],
  },
  {
    id: "history",
    title: "Note Version History",
    tags: ["recovery"],
    content: "Meaningful saves create immutable recoverable versions.",
    links: ["conflicts"],
  },
  {
    id: "attachments",
    ...
    tags: ["files", "s3"],
    content: "Private object storage holds signed attachment URLs.",
    links: [],
  },
  {
    id: "graph",
    ...
    content: "Links and backlinks form navigable note relationships.",
    links: ["offline", "auth"],
  },
  {
    id: "editor",
    ...
    tags: ["writing"],
    content: "The editor uses a textarea with Markdown shortcuts.",
    links: ["auth"],
  },
  {
    id: "metrics",
    ...
    tags: ["ai", "cost"],
    content: "Track request latency failures and rate limits.",
    links: [],
  },
  {
    id: "research",
    ...
    tags: ["retrieval"],
    content: "Lexical search works well for exact terminology.",
    links: ["graph"],
  },
  {
    id: "planning",
    ...
    content: "Prioritize reliability and accessibility work.",
    links: ["deployment"],
  },
  {
    id: "personal",
    title: "Weekend Plans",
    tags: ["personal"],
    content: "Walk, read, and prepare meals.",
    links: [],
  },
  {
    id: "database",
    ...
    tags: ["database"],
    content: "Indexes support ownership and note lookup.",
    links: ["auth"],
  },
  {
    id: "security",
    title: "Input Security Review",
    tags: ["security"],
    content: "Validate MIME types and treat content as untrusted.",
    links: ["attachments"],
  },
  {
    id: "testing",
    ...
    tags: ["qa"],
    content: "Run lint builds API tests and mobile checks.",
    links: ["deployment"],
  },
  {
    id: "deleted",
    ...
    tags: ["offline"],
    content: "Stale draft should never be retrieved.",
...
const queries = [
  ...
  },
  {
    id: "paraphrase-files",
    category: "paraphrase",
    question: "Where are uploaded files kept?",
  ...
  {
    id: "synonym-deployment",
    category: "synonym",
    question: "hosting production",
    expected: ["deployment"],
  },
  {
    id: "conceptual-security",
    category: "conceptual",
    question: "How do I stop unauthorized people seeing private information?",
    expected: ["auth", "security"],
  },
  {
    id: "conceptual-recovery",
    category: "conceptual",
    question: "How do changes avoid being overwritten?",
    expected: ["conflicts", "history"],
  },
  {
    id: "multi-topic",
    category: "multi-topic",
    question: "offline conflicts and version recovery",
    expected: ["offline", "conflicts", "history"],
  },
  {
    id: "relationship",
    category: "relationship",
    question: "What connects offline work to recovery?",
    expected: ["offline", "conflicts", "history"],
  },
  {
    id: "ambiguous",
  ...
  },
  {
    ...
    question: "autenticación de sesiones",
    expected: ["auth"],
  },
];
...
function hybridRank(question, limit = 5) {
  ...
  return notes
    ...
    .map((note) => {
      ...
      const relationship = note.links.some((id) =>
        queryTokens.some((token) => id.includes(token)),
      )
        ? 2
        : 0;
...
function metrics(retriever, limit) {
  const rows = queries.map((query) => {
    ...
    const firstRelevant = retrieved.findIndex((id) =>
    ...
    return {
      ...
      recall: query.expected.length
        ? relevant.length / query.expected.length
        : 0,
...
test("expanded lexical and test-only hybrid benchmark reports quality by query class", () => {
  ...
  const byClass = Object.fromEntries(
    [...new Set(queries.map((query) => query.category))].map((category) => [
      category,
      {
        lexicalHit: lexical.rows
          .filter((row) => row.category === category)
          .filter((row) => row.hit).length,
        hybridHit: hybrid.rows
          .filter((row) => row.category === category)
          .filter((row) => row.hit).length,
  ...
  console.log(
    ...
    JSON.stringify({ lexical, hybrid, byClass }),
  );
