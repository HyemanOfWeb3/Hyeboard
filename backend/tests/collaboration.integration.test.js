...
test.beforeEach((context) => {
  if (!enabled)
    context.skip("Requires isolated TEST_MONGO_URI and TEST_AUTH_SECRET");
});
...
before(async () => {
  ...
  note = await Note.create({
    ...
    clientNoteId: "collab-shared-note",
  });
  ...
  await editorAgent
    .post("/api/auth/login")
    .send({ email: editor.email, password: "test-password-123" })
    .expect(200);
  await viewerAgent
    .post("/api/auth/login")
    .send({ email: viewer.email, password: "test-password-123" })
    .expect(200);
});
...
test("owner can invite editor and viewer, permissions are enforced, and revocation removes access", async () => {
  if (!enabled) return;
  const editorInvite = await ownerAgent
    .post(`/api/collaboration/notes/${note._id}/invitations`)
    .send({ email: editor.email, role: "editor" })
    .expect(201);
  const viewerInvite = await ownerAgent
    .post(`/api/collaboration/notes/${note._id}/invitations`)
    .send({ email: viewer.email, role: "viewer" })
    .expect(201);
  await editorAgent
    .post("/api/collaboration/invitations/accept")
    .send({ token: editorInvite.body.token })
    .expect(200);
  await viewerAgent
    .post("/api/collaboration/invitations/accept")
    .send({ token: viewerInvite.body.token })
    .expect(200);
  ...
  await editorAgent
    ...
    .send({
      ...
      content: "Edited",
      baseRevision: 1,
      operationId: "collab-editor-update",
    })
    .expect(200);
  await viewerAgent
    .put(`/api/notes/${note._id}`)
    .send({
      ...
      content: "Denied",
      baseRevision: 2,
      operationId: "collab-viewer-update",
    })
    .expect(403);
  await ownerAgent
    .delete(`/api/collaboration/notes/${note._id}/members/${viewer._id}`)
    .expect(200);
